...
`Statement.lean` imports only Mathlib, so the claim cannot depend on any project definition. The proof in [`comparator/Solution.lean`](comparator/Solution.lean) uses only the axioms `propext`, `Quot.sound`, `Classical.choice`; this was confirmed by an independent kernel replay with Lean Comparator on 2026-07-28 (see [`comparator/README.md`](comparator/README.md)).
