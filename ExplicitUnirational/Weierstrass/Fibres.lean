...
/-!
# Irreducible singular fibres ([CLOP Lemma 2.2], [CLOP Lemma 4.1], [CLOP Lemma 4.2])

## Scope — deliberate replacement of Tate / Kodaira

The paper classifies singular fibres of the three elliptic surfaces by Tate's algorithm:
two of type II and eight of type I₁ over `ℚ` ([CLOP Lemma 4.1]); three of type II and six of
type I₁ after reduction mod 5 ([CLOP Lemma 4.1]); two of type II and eight of type I₁ for the
`C(t)` family ([CLOP Lemma 4.2]). **This module does not formalize Tate's algorithm**, nor the
Kodaira symbols themselves.

What the downstream argument actually consumes — and the only consequence of the fibre
classification used anywhere in the paper — is the Shioda–Tate hypothesis ([CLOP §2,
Shioda–Tate], [CLOP Cor 3.8]):
...
emphasize the singular case, and apply it to the three families of the paper.

The discriminant factorizations `Δ = −16(4A³ + 27B²) = −u²v²Q₈` and
`4A³ + 27B² = 27u²v²Q₊Q₋` (computed here; not displayed in [CLOP]) are re-proved here as ring
...
2. **Arithmetic family over `ℚ`.** The model with `A = u v² (3u − v)`, `B = ¼ u v P₄`
   (the `a₄`, `a₆` of [CLOP §1, S_ℚ]) at every base point — in particular
   at every singular fibre — is geometrically irreducible.
3. **Reduction mod 5 and the `C(t)` family.** The same statement for the coefficients
   specialized mod 5 ([CLOP Lemma 4.1]) and for the `C(t)` Weierstrass model of
   [CLOP §4.2] with its discriminant factors `Q±` (not displayed in [CLOP]).
-/
...
/-- The short-form discriminant `Δ = −16 (4 A³ + 27 B²)`. -/
...
At `u = 0` or `v = 0` the paper finds type II ([CLOP Lemma 4.1]): the fibre is the cuspidal
cubic `y² = x³`.
...
/-! ## Arithmetic family (the `a₄`, `a₆` of [CLOP §1, S_ℚ] and its discriminant)

`A = u v² (3u − v)`, `B = (1/4) u v P₄(u,v)`. -/

/-- The bihomogeneous quartic `P₄(u,v)`, the quartic factor of `a₆` in [CLOP §1, S_ℚ],
evaluated at ring elements. -/
public def P4_eval {R : Type*} [CommRing R] (u v : R) : R :=
  4 * v ^ 4 - 23 * u * v ^ 3 - 18 * u ^ 2 * v ^ 2 + u ^ 3 * v - 4 * u ^ 4

/-- The bihomogeneous octic `Q₈(u,v) = 64 u v⁴ (3u − v)³ + 27 P₄²`, the discriminant octic of
the model [CLOP §1, S_ℚ] (not displayed in [CLOP]), evaluated at ring elements. -/
public def Q8_eval {R : Type*} [CommRing R] (u v : R) : R :=
  64 * u * v ^ 4 * (3 * u - v) ^ 3 + 27 * (P4_eval u v) ^ 2

/-- Coefficient `A` of the arithmetic Weierstrass family (the `a₄` of [CLOP §1, S_ℚ]). -/
public def A_arith {R : Type*} [CommRing R] (u v : R) : R :=
  u * v ^ 2 * (3 * u - v)

/-- Coefficient `B` of the arithmetic Weierstrass family (the `a₆` of [CLOP §1, S_ℚ]). -/
public def B_arith {R : Type*} [Field R] (u v : R) : R :=
  (u * v * P4_eval u v) / 4

/-- Integral form of the discriminant factorization (not displayed in [CLOP]):
`-64 A³ − 27 W² = −u² v² Q₈` with `W = u v P₄`. -/
...
/-- Clearing the `/4` in `B`: short-form `Δ` matches the integral expression of the
...
This is the Shioda–Tate input of [CLOP Lemma 2.2] / [CLOP §2, Shioda–Tate]. -/
...
/-! ## Reduction modulo 5 ([CLOP Lemma 4.1])
...
and six type-I₁ fibres of [CLOP Lemma 4.1]). -/
...
/-- Type-II places of [CLOP Lemma 4.1]: `[0:1]` and `[1:0]` give the cuspidal cubic. -/
...
/-! ## Geometric family over `ℚ(t)` ([CLOP §4.2])

`A = −3 u² v²`, `B = −u⁵ v − u v⁵ − 27 t² u² v⁴`. The discriminant factors as
`27 u² v² Q₊ Q₋` (computed here; not in [CLOP]). Every specialized fibre is an irreducible
Weierstrass cubic. -/

/-- Coefficient `A` of the `C(t)` Weierstrass family ([CLOP §4.2, the equation for J_η]). -/
...
/-- The discriminant factor `Q₊(u,v) = u⁴ + v⁴ + 27 t² u v³ + 2 u² v²` (not in [CLOP]),
evaluated. -/
...
/-- The discriminant factorization specialized: `4 A³ + 27 B² = 27 u² v² Q₊ Q₋`. -/
...
([CLOP Lemma 2.2] / [CLOP Lemma 4.2]). Working over `ℚ(t)` via `RatFunc ℚ`. -/
...
The three statements consumed by Shioda–Tate in the paper. -/

/-- Fibre half of [CLOP Lemma 2.2] / [CLOP Lemma 4.1]: every geometric singular fibre of the
arithmetic surface is irreducible. -/
...
/-- Fibre half of [CLOP Lemma 4.1] (mod-5 case): every geometric singular fibre of the mod-5
...
/-- Fibre half of [CLOP Lemma 2.2] / [CLOP Lemma 4.2]: every geometric singular fibre of the
`C(t)` surface is irreducible. -/
