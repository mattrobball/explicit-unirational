...
/-!
# Discriminant factorizations for the pencils of `[CLOP §4.1]` and `[CLOP §4.2]`

Pure polynomial identities over `ℚ`:

* the quartic `P4` (the quartic factor of `a₆` in `[CLOP §1, S_ℚ]`), the discriminant octic
  `Q8`, the Weierstrass discriminant factorization `Δ = -u² v² Q8`, and the dehomogenized
  octic `q8` with the three numeric values used to place its roots (none of these
  factorizations are displayed in [CLOP]);
* the analogous factorization `4A³ + 27B² = 27 u² v² Q₊ Q₋` over `ℚ(t)` for the pencil of
  `[CLOP §4.2]` (also not displayed in [CLOP]), encoded in `MvPolynomial (Fin 3) ℚ` with
  variables `(u,v,t) = (X 0, X 1, X 2)`.

All identities are characteristic-free and close by `ring` (after clearing the rational
scalar in `B` for the factorization `Δ = -u² v² Q8`).
...
/-! ## Arithmetic surface: `P4`, `Q8`, and the discriminant -/
...
/-- The discriminant octic `Q₈(u,v)` of the model `[CLOP §1, S_ℚ]` (not displayed in [CLOP]).
Variables: `u = X 0`, `v = X 1`. -/
public noncomputable def Q8 : MvPolynomial (Fin 2) ℚ :=
  64 * X 0 * (X 1) ^ 4 * (3 * X 0 - X 1) ^ 3 + 27 * P4 ^ 2

/-- The discriminant of the Weierstrass model over `ℚ` factors as `-u² v² Q₈`
(not displayed in [CLOP]).

With `A = u v² (3u - v)` and `W = u v P₄` (the coefficients `a₄`, `a₆` of
`[CLOP §1, S_ℚ]` after completing the square, so `B = W / 4`), one has
...
/-! ## Dehomogenized octic `q₈` (not displayed in [CLOP]) -/

/-- The dehomogenized octic `q₈(z)`, given by its expanded form (not displayed in [CLOP]).
...
/-- The expanded form of the dehomogenization of `Q8` at `u = 1`. -/
...
/-- The three numeric values used to place the roots of `Q8`. -/
...
/-! ## Geometric surface over `ℂ(t)`: `Q±` and the discriminant factorization (not in [CLOP])

Variables are `(u, v, t) = (X 0, X 1, X 2)` in `MvPolynomial (Fin 3) ℚ`.
-/

/-- `Q₊(u,v) = u⁴ + v⁴ + 27 t² u v³ + 2 u² v²`, in variables `(u,v,t) = (X 0, X 1, X 2)`. -/
...
/-- With `A = -3 u² v²` and `B = -u⁵ v - u v⁵ - 27 t² u² v⁴`,
`4A³ + 27B² = 27 u² v² Q₊ Q₋`. -/
