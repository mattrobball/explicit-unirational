...
/-!
# Nine-cycle Frobenius and arithmetic Picard ranks (note §4.2–4.3)

Pure representation-theoretic content of Proposition 4.2 and Corollary 4.3, independent of
geometry:

* the geometric sum `∑_{i=0}^{8} X^i` factors as `Φ₃ Φ₉` over `ℚ` (note (4.5));
* `Nat.gcd 9 n` realises the three cases of formula (1.3);
* geometric Frobenius acts as the 9-cycle `finRotate 9` on the nine base points.

The characteristic polynomial claim of Proposition 4.2 is the identity
`(T^9 - 1)/(T - 1) = Φ₃(T) Φ₉(T)`, formalised as `cyclotomic_three_mul_nine`.
Corollary 4.3 reduces formula (1.3) to the value of `gcd 9 n`, formalised as
`picardRank_formula`.
-/
...
/-! ## Cyclotomic factorization (note (4.5)) -/
...
/-- Note (4.5): `Φ₃(T) Φ₉(T) = ∑_{i=0}^{8} T^i` over `ℚ`. -/
...
/-! ## Arithmetic Picard rank cases (note (1.3) / Corollary 4.3) -/
...
the arithmetic Picard rank of note eq. (1.3). -/
...
/-! ## Nine-cycle on the base points (note Proposition 4.2 setup) -/
