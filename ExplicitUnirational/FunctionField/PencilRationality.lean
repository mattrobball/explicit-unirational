...
/-!
# Rationality of the incidence surface of a cubic pencil ([CLOP §3, `Y = Bl_Σ ℙ²`])

Field-theoretic form of the rationality of the incidence surface, which is built into the
construction of [CLOP §3] (`Y = Bl_Σ ℙ²` for `Z = ℙ²`), for the pencils of [CLOP §4.1] and
[CLOP §4.2]: on the chart `Z = 1`, the pencil relation
...
/-! ## Rationality of the incidence surface ([CLOP §3, `Y = Bl_Σ ℙ²`]), field-theoretic form -/

/-- [CLOP §3, `Y = Bl_Σ ℙ²`]: the incidence relation is solved by `z = -f₀ / f₁` in the
rational function field of the affine plane, and adjoining that parameter does not enlarge
the field generated by the two plane coordinates.
...
/-! ## Concrete pencils from [CLOP §4] -/

/-- Homogeneous forms of the rational pencil of [CLOP §4.1]. Indices `0,1,2` correspond to `X,Y,Z`:
`F₀ = Y Z² - X³`. -/
...
/-- Homogeneous forms of the rational pencil of [CLOP §4.1]:
`F₁ = X Z² + Y³ + Y² Z - Z³`. -/
public noncomputable def F₁_rat : MvPolynomial (Fin 3) ℚ :=
...
/-- Homogeneous forms of the `C(t)` pencil of [CLOP §4.2]. Indices `0,1,2` correspond to `X,Y,Z`:
...
/-- Homogeneous forms of the `C(t)` pencil of [CLOP §4.2]:
